...
export default function CustomerLoginPage() {
  ...
  const onSubmit = async (data: any) => {
    setError("");
    try {
      const res = await fetchStorefrontApi(shopSlug, '/auth/login/', {
        method: "POST",
  ...
  return (
    <div className="max-w-md mx-auto mt-20 p-8 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
      <h1 className="text-2xl font-bold font-outfit text-center mb-2">Welcome Back</h1>
      <p className="text-center text-slate-500 mb-8">Sign in to your account to checkout.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          ...
          <input type="password" {...register("password", { required: true })} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" required />
        </div>
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        Don't have an account? <Link href={getStorefrontLink('/register', shopSlug)} className="text-indigo-600 hover:underline font-medium">Sign up</Link>
      </p>
    </div>
  );
}
